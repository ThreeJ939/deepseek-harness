...
function meta(version: string, failure: RemoteFailure | undefined): ResourceSnapshot<WorkspaceFileStat> {
  const value: WorkspaceFileStat = { absolutePath: ABSOLUTE_PATH, version, bytes: 100 }
...
export interface Harness {
  ...
  /**
   * Script whether the next render's `useResource` reports a newer Host version
   * than the pages hold (`true` → `v2`, `false` → `v1`).
   */
...
export function harness(script: Record<number, RemoteResult<WorkspaceFileText>> = {}): Harness {
  ...
  const current = { version: 'v1', failure: undefined as RemoteFailure | undefined, snapshot: meta('v1', undefined) }
  ...
  return {
    ...
    setChanged(changed) {
      // A pending change is a Host version ahead of the pages; clearing it
      // aligns metadata with whatever version the store currently holds.
      current.version = changed ? 'v2' : (instance.getSnapshot().byTab[TAB_ID]?.version ?? 'v1')
      refresh()
    },
