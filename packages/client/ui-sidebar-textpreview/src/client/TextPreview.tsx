 ...
 * metadata — its absolute path and version — and this type's own store holds
 * the pages it read through its face. A Host-reported version advance is
...
export function TextPreview({
  ...
  // Pages belong to one version; a newer Host version (or a failed metadata
  // frame) leaves them on screen until the reader asks to re-read.
  const stale = meta.value !== undefined
    && state.version !== undefined
    && meta.value.version !== state.version
  const reload = (): void => { reloadPages(tab.id, file, signal) }
  return (
