...
- **Mutually exclusive with document preview.** Both packages register Sidebar tab kind `text` at the `fallback` band; the shipped web-app mounts `@deepseek-ai/dsh-client-ui-sidebar-documentpreview` instead. Do not load both in one composition.
