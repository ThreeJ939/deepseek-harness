...
<a id="archived-downloads"></a>
### Archived downloads

The same presets expose `archive_deliverable` for immutable attachment copies. By default a successful `present` also archives automatically. Same-path archives share the present card’s Download control; archive-only paths still render a dedicated download card. The browser `GET`s `/api/deliverable.download?sessionId=&seq=&index=` with the multi-user Bearer JWT when `dsh.auth.jwt` is present, buffers the response as a blob, and saves it through a temporary object URL so `<a download>` does not need Authorization. Authorization still uses the archived Session event coordinates; bare attachment ids are refused. HTTP failures surface on the card; a 401 clears the stored JWT and notifies the login UI. Download requires a mounted attachment store; without one the Host does not register the route, and auto-archive blocks present when that switch is on.
...
Native opening uses an authenticated POST addressed by the viewed Session, event sequence, and original file index. The Host reads the viewed Session header with the declaration and passes its cwd, or the deployment workspace root when absent, to `workspaceFiles.stat`. This uses the same composed filesystem as Sidebar previews and does not activate an Agent, including for child Sessions. Native actions require the canonical process path to map from a Host path back to that same process path. Providers without this mapping return 422 and the card directs the user to Sidebar preview; a same-named Host file is insufficient. The same configured desktop availability governs metadata and execution. Edits affect subsequent opens; deletion returns an error. No file-content copy or attachment is created for present. Plugin disposal cancels and awaits pending native-open requests. Archived downloads use an authenticated GET/HEAD that loads a `deliverables/archived` entry by coordinates and streams attachment bytes; the route is registered only while an attachment store is mounted.
...
- **Archived downloads require attachment storage** — without a mounted attachment store the download route is absent; attachments are never deleted automatically, and the first release does not issue presigned object URLs.
- **Archived downloads buffer in the browser** — the Client authenticates a GET and materializes a blob before save, so very large archives compete for tab memory; streaming `<a href>` downloads cannot carry Bearer headers under multi-user JWT auth.
...
**Runtime invariant:** No companion is published. Prompt, slot, dictionary, file-action route, archive-download route, and optional service registrations are effect-owned; the Session log owns declarations and archive references, the filesystem owns editable source contents, and the attachment store owns archived bytes.
