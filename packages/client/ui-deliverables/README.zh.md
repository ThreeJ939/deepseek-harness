...
同一套 preset 提供 `archive_deliverable` 用于不可变附件副本。默认情况下成功的 `present` 也会自动归档。同路径归档与 present 卡共用「下载」控件；仅有归档、无 present 的路径仍单独显示下载卡。浏览器对 `/api/deliverable.download?sessionId=&seq=&index=` 发起 `GET`，在存在 `dsh.auth.jwt` 时附带多用户 Bearer JWT，将响应缓冲为 blob，再经临时 object URL 保存，因而 `<a download>` 无需 Authorization。授权仍使用归档 Session 事件坐标；拒绝裸 attachment id。HTTP 失败显示在卡片上；401 会清除已存 JWT 并通知登录 UI。下载需要已挂载的附件存储；缺失时 Host 不注册该路由，且在自动归档开关开启时会 block present。
...
原生打开使用经过认证的 POST，通过当前查看的会话、事件序号和原始文件索引定位声明。Host 读取声明及当前查看的会话 header，将其中的 cwd 传给 `workspaceFiles.stat`；未记录 cwd 时使用部署的工作目录。它与侧栏预览使用同一组合文件系统，无需启动 Agent，子会话也适用。原生操作要求规范化的进程路径能从 Host 路径映射回同一进程路径。提供方没有这种映射时返回 422，卡片提示使用侧栏预览；Host 上存在同名文件并不足够。同一份桌面可用性配置同时约束信息查询和实际执行。编辑会影响后续打开的内容；删除后返回错误。不创建文件内容副本或附件。插件释放时取消并等待进行中的原生打开请求。归档下载使用经过认证的 GET/HEAD，按 `deliverables/archived` 坐标取出 `FileAttachmentRef` 并流式返回附件字节；未挂载附件存储时不注册该路由。
...
- **归档下载需要附件存储**——未挂载附件存储时不注册下载路由；附件不会自动删除，第一版也不签发预签名对象 URL。
- **归档下载在浏览器中缓冲**——Client 先鉴权 GET 再物化为 blob 后保存，很大的归档会占用标签页内存；在多用户 JWT 鉴权下，流式 `<a href>` 下载无法携带 Bearer 头。
...
**运行时不变式：** 不发布伴生入口。提示词、slot、dictionary、文件操作路由、归档下载路由与可选 service 注册归 effect 所有；Session 日志拥有声明与归档引用，文件系统拥有可编辑源文件内容，附件存储拥有归档字节。
