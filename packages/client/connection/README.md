...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Each process mints a random launch token. `dsh-web-app` prints and opens the ordinary root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `/`. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no query token outside the root exchange and no Authorization-header token. When `dsh-host-auth-middleware` is composed (`dsh-multi-user`), `authorizeIndex` serves the SPA without that process-token exchange so the login UI can load; `/api` then requires a JWT principal (or still accepts the process cookie for single-user tooling on paths that reach Connection before a principal is bound). See [same-process multi-tenant](../../../.agents/notes/implemented/architecture/2026-08-27-same-process-multi-tenant.md) and [multi-user SPA index auth](../../../.agents/notes/implemented/bug-fix/2026-09-10-multi-user-spa-index-auth.md).
