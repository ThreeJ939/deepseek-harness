...
每个 Host RPC 方法和 WebSocket stream 都要求同一个浏览器会话，不存在按方法区分的 loopback 层。每个进程生成一个随机启动令牌。`dsh-web-app` 打印并打开带 `?token=...` 的普通根 URL；`frontend-static` 把根路径和 index 请求交给 `ctx.connection.authorizeIndex`，后者只在 `GET /` 接受该令牌，写入绑定 authority 的签名 cookie，再重定向到干净的 `/`。缺失、过期、畸形或 authority 不匹配的 cookie 会在 RPC 分发前得到 401。静态资源保持公开。HTTP 载体不在根路径交换之外接受 query token，也不接受 Authorization header token。当组合了 `dsh-host-auth-middleware`（`dsh-multi-user`）时，`authorizeIndex` 在没有 process-token 交换的情况下仍提供 SPA，以便登录 UI 可以加载；随后 `/api` 需要 JWT 主体（在主体尚未绑定、请求仍到达 Connection 的路径上，仍接受 process cookie 以兼容单用户工具）。见[同进程多租户](../../../.agents/notes/implemented/architecture/2026-08-27-same-process-multi-tenant.zh.md)与[多用户 SPA 首页鉴权](../../../.agents/notes/implemented/bug-fix/2026-09-10-multi-user-spa-index-auth.zh.md)。
