...
export function LoginPage({ config, t, onSuccess }: LoginPageProps) {
  const safeConfig: Config = config ?? { allowJwtPaste: false }
  const [pasteValue, setPasteValue] = useState('')
  const [pasteOpen, setPasteOpen] = useState(safeConfig.allowJwtPaste === true && (
    safeConfig.oauthAuthorizationEndpoint === undefined
    || safeConfig.oauthClientId === undefined
    || safeConfig.oauthRedirectUri === undefined
  ))
  const [error, setError] = useState<string | undefined>()
  const [exchanging, setExchanging] = useState(false)

  const hasOAuth = safeConfig.oauthAuthorizationEndpoint !== undefined
    && safeConfig.oauthClientId !== undefined
    && safeConfig.oauthRedirectUri !== undefined
  const showPasteToggle = hasOAuth && safeConfig.allowJwtPaste === true
  const showPasteForm = safeConfig.allowJwtPaste === true && (pasteOpen || !hasOAuth)
  ...
  useEffect(() => {
    ...
    if (
      safeConfig.oauthTokenEndpoint === undefined
      || safeConfig.oauthClientId === undefined
      || safeConfig.oauthRedirectUri === undefined
    ) {
    ...
    exchangeCodeForJwt(
      code,
      codeVerifier,
      safeConfig.oauthTokenEndpoint,
      safeConfig.oauthClientId,
      safeConfig.oauthRedirectUri,
      t,
  ...
  const handleOAuthClick = useCallback(async () => {
    ...
    try {
      const { url, state, codeVerifier } = await buildAuthorizationUrl(
        safeConfig.oauthAuthorizationEndpoint,
        safeConfig.oauthClientId,
        safeConfig.oauthRedirectUri,
        safeConfig.oauthScopes ?? 'openid profile',
      )
    ...
    <div className={css.shell}>
      ...
      {exchanging ? (
        ...
        <div className={css.actions}>
          ...
          {!hasOAuth && safeConfig.allowJwtPaste !== true && (
