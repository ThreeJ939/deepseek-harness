...
export function apply(ctx: Context, config: AuthUiConfig): void {
  ...
  // Structural row matches dsh-host-webserver's `kind: 'global'` IndexInjection.
  ctx.inject(['webServer'], (scope) => {
    scope.effect(() => (scope.on as (event: string, listener: (...args: never[]) => void) => () => void)(
      'webserver/index-inject',
      ((table: Array<{ kind: string; name?: string; value?: unknown }>) => {
