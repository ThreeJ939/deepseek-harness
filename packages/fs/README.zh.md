...
九个包加上远程同级 `fs-e2b` 承担文件系统角色；子系统参考文档完整收录各项约定与错误分类体系。
...
| [`tool-present/`](tool-present/README.zh.md) | 显式声明可访问的工作区源文件交付 | 注册到 `ctx.tools` |
| [`tool-deliverable-archive/`](tool-deliverable-archive/README.zh.md) | 将工作区文件归档进持久附件存储以供下载 | 注册到 `ctx.tools` |
