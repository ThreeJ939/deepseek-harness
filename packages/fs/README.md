...
| [`tool-present/`](tool-present/README.md) | Explicit workspace source-file delivery declarations | registers on `ctx.tools` |
| [`tool-deliverable-archive/`](tool-deliverable-archive/README.md) | Archive workspace files into durable attachment storage for download | registers on `ctx.tools` |
