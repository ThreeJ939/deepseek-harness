...
import SessionStore, { SessionSeq, type SessionEvent } from '@deepseek-ai/dsh-session'
...
async function verifyBackend(
  ...
  try {
    ...
    try {
      expect({ meta: reader.header, events: (await reader.read()).events }, name).toEqual({ meta: header, events })
    } finally {
      await reader.close()
    }
    const listed = await mounted.persistence.list()
    expect(listed.map(snapshot => snapshot.header), name).toEqual([header])
    const revision = listed[0]?.revision
    const stored = await readerEvents(mounted.persistence, header.id)
    for (let fromSeq = 0; fromSeq <= events.length + 1; fromSeq += 1) {
      expect(stored.filter(event => event.seq >= fromSeq), `${name} seq ${fromSeq}`)
        .toEqual(events.slice(fromSeq))
    }
    expect((await mounted.persistence.list())[0]?.revision, name).toBe(revision)
  } finally {
  ...
  try {
    const reader = await mounted.persistence.open(header.id, 'read')
    try {
      expect({ meta: reader.header, events: (await reader.read()).events }, `${name} reopen`).toEqual({ meta: header, events })
    } finally {
      await reader.close()
    }
  } finally {
...
async function readerEvents(
  persistence: SessionPersistence,
  id: ReturnType<typeof meta>['id'],
