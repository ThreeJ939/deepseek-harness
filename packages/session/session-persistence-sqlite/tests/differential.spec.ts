...
async function verifyBackend(
  ...
  try {
    ...
    try {
      expect({ meta: reader.header, events: await reader.read() }, name).toEqual({ meta: header, events })
    } finally {
      await reader.close()
    }
  ...
  try {
    const reader = await mounted.persistence.open(header.id, 'read')
    try {
      expect({ meta: reader.header, events: await reader.read() }, `${name} reopen`).toEqual({ meta: header, events })
    } finally {
      await reader.close()
    }
  } finally {
