...
function event(seq: number, time: number, value: StreamChunk, turn = 1, step = 1): SessionEvent {
  // 'assistant/chunk' was removed from SessionEventMap in V3; cast simulates stored schema-17 historic rows.
  return { type: 'assistant/chunk', seq: SessionSeq(seq), time, data: { turn, step, chunk: value } } as unknown as SessionEvent
}
...
describe('SQLite compression', () => {
  ...
  it('keeps every off-format delta scalar and splits incompatible runs', () => {
    const malformed = (seq: number, data: unknown): SessionEvent => ({
      type: 'assistant/chunk', seq: SessionSeq(seq), time: 10 + seq, data,
    } as unknown as SessionEvent)
    const values: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } },
      { ...chunk(1), extra: true } as unknown as SessionEvent,
      { ...chunk(-1), seq: SessionSeq(-1) } as SessionEvent,
      { ...chunk(3), time: 1.5 },
      malformed(4, 'data'),
