...
function chunk(seq: number, text = `token-${seq}`): SessionEvent {
  // 'assistant/chunk' was removed from SessionEventMap in V3; cast simulates stored schema-17 historic rows.
  return {
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeThrough(ctx.sessionPersistence, header, events)
    expect((await readThrough(ctx.sessionPersistence, header.id)).events).toEqual(events)
    await ctx.fiber.dispose()
  ...
  it('packs each append once without rewriting earlier rows and seeks inside packed rows', async () => {
    ...
    const handle = await ctx.sessionPersistence.create(header)
    await handle.append(events.slice(0, 3))
    await handle.append(events.slice(3, 4))
    const before = new DatabaseSync(path, { readOnly: true })
    const originalRows = before.prepare(testSql('select-event-rowids')).all()
    before.close()
    await handle.append(events.slice(4))
    const { events: inspected } = await handle.read()
    await handle.close()
    expect(inspected).toEqual(events)
    const stored = await readThrough(ctx.sessionPersistence, header.id)
    for (const fromSeq of [0, 2, 25, 101, 104, 105]) {
      expect(stored.events.filter(event => event.seq >= fromSeq))
  ...
  it('includes a packed predecessor when an overlapping scalar tail hides it', async () => {
    ...
    await store.appendBatch(header, [chunk(0), chunk(1), chunk(2)], false, SessionLogOffset(0))
  ...
  it('waits for a competing process within the configured busy timeout', async () => {
    ...
    await store.appendBatch(header, [chunk(0)], false, SessionLogOffset(0))
    ...
    try {
      ...
      await expect(store.appendBatch(header, [chunk(1)], true, SessionLogOffset(0))).resolves.toBeUndefined()
  ...
  it('rejects a stale physical append without replacing the winning tail', async () => {
    ...
    await first.appendBatch(header, [chunk(0)], false, SessionLogOffset(0))
    await second.appendBatch(header, [chunk(1)], true, SessionLogOffset(0))
    await expect(first.appendBatch(header, [chunk(1)], true, SessionLogOffset(0))).rejects.toThrow(/stored next seq is 2/)
  ...
  it('rejects a stale repair without deleting a newer winning tail', async () => {
    ...
    await stale.appendBatch(header, [chunk(0)], false, SessionLogOffset(0))
    const db = new DatabaseSync(path)
    ...
    await winner.appendBatch(header, [chunk(1), chunk(2)], true, SessionLogOffset(0))
...
describe('SessionPersistenceSqlite schema ownership', () => {
  ...
  it('validates creation time and restores every optional header field', () => {
    ...
    expect(() => decodeSessionRow({ ...base, owner_id: '' })).toThrow(/owner_id/)
  })
  ...
  it('rejects malformed SQLite row primitives generically', () => {
    ...
    for (const [value, message] of [
      ...
      [{ ...base, owner_id: 1 }, /owner_id.*string or null/],
    ] as const) {
...
describe('SessionPersistenceSqlite edge behavior', () => {
  ...
  it('uses constructor defaults and writes through a handle', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    let persistence!: SessionPersistenceSqlite
    await ctx.plugin(Object.assign((inner: Context) => {
      persistence = new SessionPersistenceSqlite(inner, { path: ':memory:' })
    }, { name: 'session-persistence-sqlite-direct' }))

    const header = meta('direct-provider')
    const events = chunkLog(3)
    await writeThrough(persistence, header, events)
    expect((await readThrough(persistence, header.id)).events).toEqual(events)
    await ctx.fiber.dispose()
  })
  ...
  it('keeps empty mutations inert and rolls back a repair without metadata', async () => {
    ...
    await store.appendBatch(header, [], false, SessionLogOffset(0))
  ...
  it('rejects malformed physical tail rows before appending', async () => {
    ...
    await expect(store.appendBatch(header, [chunk(2)], true, SessionLogOffset(0))).rejects.toThrow(/invalid physical tail/)
    await store.close()
  })
