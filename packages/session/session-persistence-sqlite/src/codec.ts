...
/** Pre-V3 assistant/chunk event shape stored in schema-17 rows; decoded read-only for historic data. */
...
function classify(event: SessionEvent): DeltaKind | undefined {
  // 'assistant/chunk' was removed from SessionEventMap in V3; the frozen codec
  // still classifies stored schema-17 rows for decoding historic data.
  const e = event as unknown as Record<string, unknown>
  if (e['type'] !== 'assistant/chunk') return undefined
...
function toolCallOf(event: DeltaEvent): { readonly id: string; readonly name?: string } {
  return event.data.chunk as unknown as { readonly id: string; readonly name?: string }
}
  ...
  if (kind === 'tool-call-delta') {
    ...
    return {
      ...
      data: {
        ...
        args: run.map(event => (event.data.chunk as unknown as { argumentsDelta: string }).argumentsDelta),
      },
    }
  }
  const data = {
    ...base,
    texts: run.map(event => (event.data.chunk as unknown as { text: string }).text),
  }
...
function emitBoundedRun(out: StorageRecord[], kind: DeltaKind, completeRun: readonly DeltaEvent[]): void {
  ...
  while (completeRun.length - offset >= MIN_PACKED_ROW_MEMBERS) {
    ...
    if (accepted === 0) {
      out.push(completeRun[offset] as unknown as StorageRecord)
      offset += 1
      continue
    }
  ...
  out.push(...completeRun.slice(offset) as unknown[] as StorageRecord[])
}
...
export function packChunkRuns(events: readonly SessionEvent[]): StorageRecord[] {
  ...
  let run: DeltaEvent[] = []
  const flush = (): void => {
    if (kind === undefined) out.push(...run as unknown[] as StorageRecord[])
...
function expandRow(row: ChunkRow): SessionEvent[] {
  ...
  for (let index = 0; index < members.length; index += 1) {
    ...
    // Pre-V3 event type; cast required since 'assistant/chunk' was removed from SessionEventMap.
    events.push({
      type: 'assistant/chunk',
      seq: (row.seq0 + index) as SessionSeq,
      time,
