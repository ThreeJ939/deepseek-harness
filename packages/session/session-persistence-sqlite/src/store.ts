...
/** SQLite physical session store: transactional append, reads, and schema checks. */
export class SqliteStore {
  ...
  async loadStored(id: SessionId, signal?: AbortSignal): Promise<StoredPrefix | undefined> {
    ...
    const meta = rowToMeta(snapshot.row)
    return {
      meta,
  ...
  /**
   * Load stored events starting from a logical sequence number.
   * Returns the session prefix with only events at `from` seq or later.
   * Packed rows that start before `from` but contain `from` are included.
   * @param id - session id to load.
   * @param from - first logical sequence number to include.
   * @param signal - optional cancellation.
   */
  async loadStoredFrom(id: SessionId, from: number, signal?: AbortSignal): Promise<StoredPrefix | undefined> {
    await this.observe(signal)
    const snapshot = this.readTransaction(() => {
      const row = this.rowFor(id)
      if (row === undefined) return undefined
      const eventRows = this.db.prepare(sql('select-events-from')).all(id, from).map(decodeEventRow)
      return { row, eventRows }
    })
    signal?.throwIfAborted()
    if (snapshot === undefined) return undefined
    ...
    return {
      meta,
      events: scanned.preserved.filter(e => e.seq >= from),
      revision: sqliteRevision(this.storeIdentity, snapshot.row),
      inheritedEventCount: SessionLogOffset(snapshot.row.seed_length ?? 0),
      ...scanned.tornFrom === undefined ? {} : { tornMarker: scanned.tornFrom },
    }
  ...
  async appendBatch(
    ...
    try {
      ...
      if (!isMaterialized) this.writeRow(meta, inheritedEventCount)
  ...
  async list(options?: { ownerUserId?: string; signal?: AbortSignal }): Promise<SessionHeader[]> {
    const signal = options?.signal
    await this.observe(signal)
    const rows = this.sessionRows(options?.ownerUserId)
    signal?.throwIfAborted()
   ...
   * @param meta - immutable session header to upsert.
  ...
  async materializeHeader(meta: SessionHeader, inheritedEventCount: SessionLogOffsetType): Promise<void> {
    ...
    try {
      validateSchemaForMutation(this.databaseConstructor, this.db, this.databasePath)
      this.writeRow(meta, inheritedEventCount)
      this.db.exec(sql('commit'))
    } catch (error: unknown) {
  ...
  async snapshot(id: SessionId, signal?: AbortSignal): Promise<SessionPersistenceSnapshot | undefined> {
    ...
    return row === undefined
      ? undefined
      : { header: rowToMeta(row), revision: sqliteRevision(this.storeIdentity, row) }
  }
  ...
  private sessionRows(ownerUserId?: string): SessionRow[] {
    if (ownerUserId === undefined) {
      return this.db.prepare(sql('select-sessions')).all().map(decodeSessionRow)
    }
    return this.db.prepare(sql('select-sessions-by-owner')).all(ownerUserId).map(decodeSessionRow)
  }
  ...
  private incrementRevision(id: SessionId): void {
    const updated = this.db.prepare(sql('update-session-revision'))
      .run(id)
    /* v8 ignore next -- materialized writes follow create()/flush; other writes upsert in this transaction. */
  ...
  private writeRow(meta: SessionHeader, inheritedEventCount: SessionLogOffsetType): void {
