...
export function bindRecord(record: StorageRecord): BoundRecord {
  ...
  const event = record
  const surfaceFields: { surfaceOp?: unknown; sourceEventSeqs?: unknown } = event
  const sourceEventSeqs = surfaceFields.sourceEventSeqs
  return {
    seq: event.seq,
    type: event.type,
    time: event.time,
    data: encodeData(JSON.stringify(event.data)),
    sourceEventSeqs: Array.isArray(sourceEventSeqs)
      ? encodeSourceEventSeqs(sourceEventSeqs as readonly number[])
      : null,
    surfaceOp: surfaceFields.surfaceOp === undefined ? null : JSON.stringify(surfaceFields.surfaceOp),
