...
describe('session-persistence-pg specifics', () => {
  ...
  it('continues append after reopen', async () => {
    ...
    try {
      expect(await reader.read()).toHaveLength(8)
    } finally {
      await reader.close()
    }
  })

  it('packs consecutive text-delta chunks into one physical row', async () => {
    const { persistence, dispose, pool } = await mount()
    disposers.push(dispose)
    const header = meta('pg-pack')
    const chunks: SessionEvent[] = [0, 1, 2, 3].map(index => ({
      type: 'assistant/chunk',
      seq: SessionSeq(1 + index),
      time: 100 + index,
      data: {
        turn: 1,
        step: 1,
        chunk: { type: 'text-delta' as const, index: 0, text: `t${index}` },
      },
    }))
    const log: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } },
      ...chunks,
      { type: 'turn/end', seq: SessionSeq(5), time: 200, data: { turn: 1, reason: { kind: 'completed' } } },
    ]
    const handle = await persistence.create(header)
    ...
    const physical = await pool.query(
      'SELECT seq, type, ignorable FROM events WHERE session_id = $1 ORDER BY seq ASC',
      [header.id],
    )
    expect(physical.rows).toEqual([
      { seq: 0, type: 'turn/start', ignorable: null },
      { seq: 1, type: 'text-chunks', ignorable: false },
      { seq: 5, type: 'turn/end', ignorable: null },
    ])
    ...
    try {
      ...
      expect(events.slice(1, 5).map(event => event.type)).toEqual([
        'assistant/chunk',
        'assistant/chunk',
        'assistant/chunk',
        'assistant/chunk',
    ...
    // Append after a packed tail must compute the next logical seq correctly.
    const writer = await persistence.open(header.id, 'write')
    await writer.append([
      { type: 'turn/start', seq: SessionSeq(6), time: 300, data: { turn: 2 } },
    ])
    await writer.flush()
    await writer.close()
    const after = await persistence.open(header.id, 'read')
    try {
      expect(await after.read()).toHaveLength(7)
    } finally {
      await after.close()
    }
  })
