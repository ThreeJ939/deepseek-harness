/**
 * The PostgreSQL provider's session storage runtime: per-session handles, one
 * in-process writer per id, routed live write-behind, and teardown.
 * @module @deepseek-ai/dsh-session-persistence-pg/storage
 */
...
/** The PostgreSQL primitives the handle drives on its owning service. */
...
/**
 * The PostgreSQL session handle. Mutations serialize on a per-handle promise
...
export class PgSessionHandle implements SessionHandle {
  ...
  /**
   * Durability barrier; materializes the session row when nothing has been appended yet.
  ...
  /**
   * Buffer one published live session event and arm the bounded batching window when idle.
...
export class PgBackendTracker {
   ...
   */
  release(handle: PgSessionHandle, materialized: boolean): void {
