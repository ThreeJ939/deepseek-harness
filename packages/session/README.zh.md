...
| [`session-persistence-sqlite/`](session-persistence-sqlite/README.zh.md) | 可选 SQLite 后端，打包 chunk 行（多用户默认） | 注册到 `ctx.sessionPersistence` |
| [`session-persistence-pg/`](session-persistence-pg/README.zh.md) | 可选 PostgreSQL 后端，每个逻辑事件一行 JSONB | 注册到 `ctx.sessionPersistence` |
