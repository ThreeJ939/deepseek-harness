...
Host 控制器会串行执行正确性取决于当前 registry 状态的变更，并为预期失败抛出带稳定 `workspace/*` 或 `directory-picker/*` 码的 `RemoteError`。它的 `follow()` 流会同步订阅持久 Workspace 变更，先发出一份完整 baseline，再按顺序发出 `upsert`、`remove`、`order` 和 `archived` 增量。组合 `auth-middleware` 时，每一代会捕获调用方主体：baseline 与 `upsert` / `remove` / `order` / `archived` 增量只包含该 viewer 拥有的 Workspace 与可归到该 viewer 的归档 Session id（与 `workspaceRegistry.list(ownerUserId)` 及 Session 所有者归属一致）；单用户组合保持未过滤的流。重连会以替换 baseline 开始新一代，因此消费方不依赖收到断线期间的每个增量。
