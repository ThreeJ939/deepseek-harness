...
function* bufferChunks(data: Uint8Array, chunkSize = 1 << 16): Iterable<Uint8Array> {
  ...
  private async projectRequest(
    ...
    if (data.byteLength > policy.maxBytes) {
      ...
      return this.requestAttachment(
        ...
        tighter.info.width,
        tighter.info.height,
        tighter.info.hasAlpha,
      )
    }
    return this.requestAttachment(stored.ref, policy, data, info.width, info.height, info.hasAlpha)
  }
