...
- **File projection is unconditional** — no provider receives file blocks; document media types expand to framed extracted text when readable, and every other file becomes one deterministic handle line so the model can read the saved copy with file tools on demand.
