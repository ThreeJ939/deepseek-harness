...
官方单用户 profile 仍只选择 JSONL。本树同时交付 `@deepseek-ai/dsh-session-persistence-sqlite`，作为多用户 bundle 的 handle 后端（[SQLite handle 持久化](../architecture/2026-09-03-sqlite-handle-session-persistence.zh.md)）。跨 package 持久化测试使用真实 JSONL provider、SQLite provider 或 owner-local fake。`@deepseek-ai/dsh-session-query-sqlite` 继续作为可选 FTS5 query provider 使用独立、可重建的数据库，`@deepseek-ai/dsh-storage-sqlite` 继续作为通用 domain-KV provider。
